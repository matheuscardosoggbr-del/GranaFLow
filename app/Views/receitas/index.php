...
<div class="d-flex justify-content-end mb-3">
    <a href="<?= BASE_URL ?>receitas/novo" class="btn btn-primary btn-acao">
...
<?php else: ?>
    <div class="card formulario-card">
        <div class="card-body">
                        ...
                        <?php foreach ($receitas as $receita): ?>
                            <tr>
                                ...
                                <td class="text-center">
                                    <a href="<?= BASE_URL ?>receitas/editar/<?= $receita['id_receita'] ?>" class="btn btn-sm btn-outline-primary btn-acao" title="Editar">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    <form method="POST" action="<?= BASE_URL ?>receitas/deletar/<?= $receita['id_receita'] ?>" style="display:inline;" onsubmit="return confirm('Deseja deletar esta receita?');">
            ...
            <hr>
            <div class="row g-2">
                <div class="col-md-6"><strong>Total de Receitas:</strong> R$ <?= number_format($total, 2, ',', '.') ?></div>
                <div class="col-md-6 text-md-end"><strong>Quantidade:</strong> <?= count($receitas) ?> receita(s)</div>
            </div>
        </div>
    </div>
<?php endif; ?>
